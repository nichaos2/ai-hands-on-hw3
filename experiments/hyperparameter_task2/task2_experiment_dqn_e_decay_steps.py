...
def run_experiment():
    # 1. Setup Directories
    epsilon_decay_steps = (
        5000  # change this to 20000 for slow and 5000 for fast (rename folder manually)
    )
    results_folder = "results_e_slow"
    # checkpoint_folder = "checkpoints_e_slow"
    if epsilon_decay_steps > 5000:
        results_folder = "results_e_fast"
        # checkpoint_folder = "checkpoints_e_fast"
    ...
    for variant_name, config in variants.items():
        ...
        if variant_name == "DQN-full":
            ...
            df_logs.to_csv("results_e_slow/dqn_logs.csv", index=False)
            ...
            pd.DataFrame(summary_data).to_csv(
                "results_e_slow/dqn_comparing_table.csv", index=False
            )
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results_e_slow/task2_dqn_epsilon.png")
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results_e_slow/task2_dqn_ablation.png")

    print("Saved ablation and epsilon plots to results_e_slow/ directory.")
